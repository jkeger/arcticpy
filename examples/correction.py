...
In this example, we'll use **arcticpy* to correct CTI in example imaging from the Hubble Space Telescope (HST)
Advanced Camera for Surveys (ACS) instrument.
"""

"""Setup the path to this example script."""
...
We use this path to set:
    config_path:
        Where arcticpy configuration files are located.
        They control many aspects of arcticpy (visualization, model priors, etc.). Feel free to check them out!.
"""
...
The _FrameACS_ contains all the functionality we need to automatically load ACS data from the above .fits structure, 
...
To correct the ACS data, we need a CTI model describing:
 ...
 - The properties of the _Trap_'s on the CCD that are responsible for CTI by capturing and releasing electrons during 
   read-out.
 - The volume-filling behaviour of electrons in CCD pixels, as this effects whether _Trap_'s are able to captrue them
   or not!
   
**arcticpy** again has in-built tools for automatically loading the CTI model appropriate for correcting ACS data, 
using the date of observation loaded in the _ACSFrame_ object to choose a model based on the date of observation.
"""

print(frame.exposure_info.modified_julian_date)

traps, ccd, roe = ac.model_for_HST_ACS(date=3000000)
...
"""
We can now pass the image and CTI model to remove CTI, which mimics the effects of CTI and in turn use this
...
frame_corrected = ac.remove_cti(image=frame, iterations=1, parallel_express=1, parallel_traps=traps, parallel_ccd=ccd)
ac.plot.Frame(frame=frame_corrected, plotter=plotter)

"""
Finally, we output this corrected image to .fits, so we can use it in the future.

[Currently, we output the image to a .fits file called 'j8xi61meq_corrected', which does NOT use the same 6 slice
multi extension fits format of the original data. We are adding a method commented out below called 'update_fits', 
which will allow the output to be in a file of this format. However this is currently bugged and disabled!]
"""
frame_corrected.output_to_fits(file_path=f"{path}/{dataset_label}/j8xi61meq_corrected", overwrite=False)